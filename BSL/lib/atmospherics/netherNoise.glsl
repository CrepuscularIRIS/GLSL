vec3 GetNetherNoise(vec3 viewPos, float VdotU, float dither) {
    float visibility = clamp(VdotU * 1.875 - 0.225, 0.0, 1.0);
    visibility *= 1.0 - VdotU * 0.75 - max(blindFactor, darknessFactor);

    if (visibility > 0.0) {
        vec3 spots = vec3(0.0);

        float eyeAltitude1 = cameraPosition.y * 0.005;
        ...
        if (abs(wpos.y) < 0.0001) wpos.y = 0.0001;
        wpos.xz /= wpos.y;
        ...
        for (int i = 0; i < sampleCount; i++) {
            ...
            vec3 noise = texture2D(noisetex, vec2(noiseSpots) + wind).g * netherCol.rgb * 2.5 - netherCol.rgb * 1.3;

            float currentM = 1.0 - current;
            spots += noise * currentM * 6.0;
        }

        return spots * visibility / float(sampleCount);
    }
