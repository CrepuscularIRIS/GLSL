/*
----------------------------------------------------------------
Photon-style Gerstner waves for still water.
Transplanted from LUX/Photon. Used by world0 (main world).
----------------------------------------------------------------
*/

#ifndef PHOTON_WAVE_SPEED_STILL
#define PHOTON_WAVE_SPEED_STILL 1.0
#endif
#ifndef PHOTON_WAVE_FREQUENCY
#define PHOTON_WAVE_FREQUENCY 1.0
#endif
#ifndef PHOTON_WAVE_PERSISTENCE
#define PHOTON_WAVE_PERSISTENCE 0.5
#endif
#ifndef PHOTON_WAVE_LACUNARITY
#define PHOTON_WAVE_LACUNARITY 1.7
#endif

const float PHOTON_TAU = 6.28318530718;
const float PHOTON_PHI = 2.39996322973;

float PhotonGerstnerWave(vec2 coord, vec2 waveDir, float t, float noise, float wavelength) {
	const float g = 9.8;
	float k = PHOTON_TAU / wavelength;
	float w = sqrt(g * k);
