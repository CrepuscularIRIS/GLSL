...
void Bloom(inout vec3 color, vec2 coord) {
	...
	netherBloom *= 1.0 - max(blindFactor, darknessFactor);
	bloomMultiplier = mix(0.7, 1.8, netherBloom);
	#elif defined END
	bloomMultiplier = 2.0;
	#endif

	#if BLOOM_CONTRAST == 0
	color = mix(color, blur, 0.2 * BLOOM_STRENGTH * bloomMultiplier);
	#else
	vec3 bloomContrast = vec3(exp2(BLOOM_CONTRAST * 0.25));
	color = pow(color, bloomContrast);
	blur = pow(blur, bloomContrast);
	vec3 bloomStrength = pow(vec3(0.2 * BLOOM_STRENGTH * bloomMultiplier), bloomContrast);
