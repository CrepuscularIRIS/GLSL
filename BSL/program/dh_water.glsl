...
float GetWaterHeightMap(vec3 worldPos, vec2 offset) {
	...
	#ifdef WORLD0
	// world0: Photon Gerstner waves (worldPos is waterPos = worldPos+cameraPosition when from GetWaterNormal)
	return GetPhotonWaterHeight(worldPos.xz + offset) * WATER_BUMP;
	#else
