...
{
	// Floor 0.2 so aurora visible at horizon (low-angle view over water)
	return max(1.0 - exp2(-cosT * 20.0), 0.2);
}
...
{
	// Fallback for loaders that may not provide worldDay (e.g. worldDay=0)
	float dayForHash = float(max(worldDay, 1)) * 239.996322973;
	float probabilityHash = Hash11(dayForHash);
	...
	// Night factor: 0.18 at full night for "always on at night", 0 at day. Reference: todo.md
	float nightFactor = (1.0 - 0.15 * rainStrength) * (1.0 - sunVisibility);
	auroraAlpha = auroraAlpha * auroraAlpha * 0.18 * Pow2(nightFactor);
