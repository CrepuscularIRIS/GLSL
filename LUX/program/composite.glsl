...
void main()
{
	...
	if (isEyeInWater == 1) 
	{
		// Sky underwater: use water fog color (Complementary-style), avoid absorption overflow
		if (z0 >= 0.9999) {
			color.rgb = Pow2(Lift(waterColor.rgb, -0.06) * (1.0 - blindFactor));
		} else {
			...
			float depthMult = clamp(GetLinearDepth(z0) * 80.0, 0.0, 60.0);
			vec3 absorption = exp2((absorptionBase - 1.0) * (12.0 + depthMult));
			float mult = 1.0 / max(GetLuminance(exp2((absorptionBase - 1.0) * 12.0)), 0.01);
