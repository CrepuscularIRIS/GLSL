...
float GetWaterHeightMap(vec3 worldPos, vec3 viewPos)
{
	...
	#if WATER_DAY_NIGHT_BLEND == 1
	mult *= mix(1.0, 1.35, sunVisibility);
	#endif
    if (mult > 0.01)
	{
		#if WATER_DAY_NIGHT_BLEND == 1
		float dayFactor = sunVisibility * (1.0 - rainStrength * 0.5);
		if (dayFactor > 0.2) {
...
void main()
{
	...
	if (albedo.a > 0.001)
	{
		...
		if (water > 0.5)
		{
			...
			#endif
			#if WATER_DAY_NIGHT_BLEND == 1
			float dayFactor = sunVisibility;
			albedo.rgb *= mix(1.0, 1.12, dayFactor);
			#endif
			albedo.a = waterAlpha;
		}
		...
		if (water > 0.5 || (translucent > 0.5 && albedo.a < 0.95))
		{
			...
			if (reflection.a < 1.0)
			{
				...
				#ifdef OVERWORLD
				// Base: sky + clouds (not just sun). Reference: todo.md
				...
				skyReflection += aurora.rgb * aurora.a;
				#endif
				...
				#if WATER_DAY_NIGHT_BLEND == 1
				float dayFactor = sunVisibility;
				bool inRain = rainStrength > 0.25;
				if (dayFactor > 0.3) {
					float skyRefMult = max(lightmap.y, 0.65) * WATER_DAY_REFLECTION_BOOST;
					skyReflection *= (4.0 - 3.0 * eBS) * skyRefMult;
				} else if (inRain) {
					float skyRefMult = max(lightmap.y, 0.55) * 1.4;
					skyReflection *= (4.0 - 3.0 * eBS) * skyRefMult;
				} else if (RAILGUN_NIGHT_REFLECTION == 1) {
					vec3 railgunNight = pow(vec3(40.0, 116.0, 255.0) / 255.0, vec3(2.2)) * 0.8;
					skyReflection = mix(railgunNight, skyReflection, max(lightmap.y, 0.45));
					skyReflection *= (4.0 - 3.0 * eBS) * max(lightmap.y, 0.6);
				} else {
					float skyRefMult = max(lightmap.y, 0.55) * 1.4;
					skyReflection *= (4.0 - 3.0 * eBS) * skyRefMult;
				}
			...
			float dayFactor = sunVisibility;
			bool inRain = rainStrength > 0.25;
			float fresnelExp = (dayFactor > 0.3) ? 0.12 : (inRain ? 0.2 : 0.18);
			#else
			float fresnelExp = 0.2;
			#endif
			albedo.rgb += reflection.rgb * pow(fresnel, fresnelExp);
