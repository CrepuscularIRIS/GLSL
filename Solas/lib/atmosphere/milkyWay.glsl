void drawMilkyWay(inout vec3 color, in vec3 worldPos, in float VoU, in float caveFactor, inout float nebulaFactor, in float occlusion) {
	...
	float visibility = mix(pow4(moonVisibility) * (1.0 - rainStrength), 1.0, altitudeFactor) * (1.0 - occlusion) * VoUFactor * MILKY_WAY_BRIGHTNESS * caveFactor;
